...
def load_bbp_data (GT,patch):
    ...
    merg=xr.merge([GT,patch],compat='no_conflicts')
    return (
        merg
        .load()
        .assign(
            input=lambda ds: ds.SPM, #replace GT with SPM when necesary  #should I be adding something here? 
            tgt=lambda ds: ds.GT,
        )[['input', 'tgt']]
        .transpose("time", "lat", "lon")
        # .transpose("nr", "lat", "lon")
        .to_array()
    )



def load_bbp_data_coarse (GT,coarse,patch):
    GT = GT.rename({'SPM': 'GT'})#bbp443 here before
    inp=patch['SPM']
    coarse = coarse['SPM'].broadcast_like(GT['GT'])
    
    ds =  (
	xr.Dataset(dict(
            input=inp, tgt=(GT['GT'].dims, GT['GT'].values), coarse=(coarse.dims, coarse.values), 
        ), inp.coords).load()
        .transpose('time', 'lat', 'lon')
    )
    return (ds.to_array())
